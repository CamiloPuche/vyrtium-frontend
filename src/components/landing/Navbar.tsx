...
export function LandingNavbar({ onSearchChange, searchValue }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-2xs">
      ...
      <div className="bg-slate-900 text-slate-300 text-[11px] py-1.5 px-4 text-center font-medium flex items-center justify-center gap-4">
        <span className="flex items-center gap-1.5">
          <Truck className="w-3.5 h-3.5 text-indigo-400" />
          <span>Envíos a todo el país · Precios en COP</span>
        </span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          {onSearchChange !== undefined && (
            <div className="relative w-48 sm:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 bg-slate-100/80 rounded-full text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all border border-slate-200/60"
              />
            </div>
          )}
          ...
          <button
            onClick={openCart}
            className="relative p-2 text-slate-700 hover:text-indigo-600 hover:bg-slate-100 rounded-full transition-colors cursor-pointer"
